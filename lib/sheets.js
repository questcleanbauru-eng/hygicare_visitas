...
// Retry com backoff só pra 429 (cota do Sheets, geralmente uma rajada curta
// de poucos segundos) — outros erros (4xx de verdade, 5xx do Google) falham
// na hora, retry não ajuda neles. Uma resposta rejeitada por cota nunca
// chega a processar a escrita/leitura do outro lado, então repetir é seguro
// mesmo pra updateRow/appendRow/deleteRow (não duplica nem perde nada).
const SHEETS_RETRY_MAX = 3;
...
async function sheetsFetch(path, options = {}, attempt = 0) {
    ...
    if (!res.ok) {
        if (res.status === 429 && attempt < SHEETS_RETRY_MAX) {
            // Backoff exponencial com jitter: ~0.4-0.7s, ~0.8-1.1s, ~1.6-1.9s.
...
// Chamado por ~10 pontos diferentes do app (ensureSheet do Radar, agendamentos,
// config, etc.) — sem cache, cada chamada era um request cheio de metadados
// da planilha inteira, mesmo pra checar 1 nome. Cache curto e compartilhado
// (não por-usuário) porque a lista de abas muda raríssimo — só quando
// alguém cria uma aba nova (ver createSheet, que já invalida na hora).
export async function sheetExists(sheetName) {
    const meta = await withCache('sheet_meta', 120, () => sheetsFetch('?fields=sheets.properties.title'));
...
// Esconde uma coluna (visualmente, no Sheets — o dado continua lá pro app
// ler normalmente) pelo nome do cabeçalho. Usado em colunas sensíveis
// (ex.: CNPJ da Base de Clientes) que precisam existir pro app cruzar dados
// mas não devem ficar visíveis de cara pra quem abre a planilha direto.
// Idempotente: reesconder uma coluna já escondida não dá erro.
export async function hideColumn(sheetName, headerLabel) {
    const headers = await getHeaders(sheetName);
    const colIndex = headers.indexOf(headerLabel);
...
// Apaga várias linhas num único batchUpdate. Ordena do fim pro começo:
// os deleteDimension da mesma request são aplicados em sequência e cada um
// desloca as linhas abaixo dele — apagando de baixo pra cima, os índices
// das linhas ainda não apagadas continuam válidos.
export async function deleteRows(sheetName, rowNumbers) {
    const uniq = Array.from(new Set((rowNumbers || []).map(Number).filter((n) => n >= 2))).sort((a, b) => b - a);
    if (!uniq.length) return;
    const sheetId = await getSheetGid(sheetName);
    await sheetsFetch(':batchUpdate', {
        method: 'POST',
        body: JSON.stringify({
            requests: uniq.map((rowNumber) => ({
                deleteDimension: {
                    range: { sheetId, dimension: 'ROWS', startIndex: rowNumber - 1, endIndex: rowNumber }
                }
            }))
...
// Promises em andamento por chave — sem isso, duas chamadas concorrentes
// pra mesma chave (comum: uma tela que dispara vários Promise.all que leem
// as mesmas abas) viam as duas o cache "frio" e disparavam a MESMA leitura
// na planilha em dobro (ou mais), contribuindo pra estourar a cota de
// requisições do Sheets. Aqui a segunda chamada só espera a primeira
// terminar, em vez de refazer o trabalho.
