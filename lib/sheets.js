...
// Como clearAndWriteColumn, mas pra abas com mais de uma coluna (ex.:
// TiposVisita, que além do nome tem TelefoneDestino/MensagemPadrao/
// Obrigatorio por linha) — reescreve a aba inteira com o cabeçalho e as
// linhas dadas, em vez de só uma coluna. Quem chama é responsável por já
// ter montado `rows` preservando os outros campos de cada linha existente.
export async function clearAndWriteTable(sheetName, headerRow, rows) {
    await sheetsFetch(`/values/${encodeRange(sheetName)}:clear`, { method: 'POST', body: '{}' });
    await sheetsFetch(
        `/values/${encodeRange(sheetName + '!A1')}?valueInputOption=USER_ENTERED`,
        { method: 'PUT', body: JSON.stringify({ values: [headerRow, ...rows] }) }
    );
}
