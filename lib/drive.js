...
export async function uploadToDrive({ folderId, name, dataUrl, base64, mimeType, kind = 'image' }) {
    ...
    if (!res.ok) {
        ...
        // Diagnóstico temporário: repassa o motivo cru do Google pro cliente.
        throw new Error(`Drive ${res.status}${reason ? ' [' + reason + ']' : ''}: ${gmsg || txt.slice(0, 200) || 'sem detalhe'} — conta: ${_saEmail || '?'}`);
    }
