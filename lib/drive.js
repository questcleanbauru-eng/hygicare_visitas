...
function getDriveClient() {
    ...
    _driveClient = new JWT({
        ...
        key: key.private_key,
        // drive.file = só os arquivos que o próprio app cria. Suficiente pra
        // subir/ler/apagar, e é o escopo mais restrito possível.
...
export async function uploadToDrive({ folderId, name, dataUrl, base64, mimeType, kind = 'image' }) {
    ...
    if (!res.ok) {
        ...
        console.error('Drive upload', res.status, txt.slice(0, 500));
        if (res.status === 403 || res.status === 404) {
            throw new Error('O app não tem acesso à pasta do Drive. Compartilhe a pasta com a conta de serviço (Editor) e ative a Drive API.');
        }
        throw new Error('Não foi possível enviar o arquivo agora.');
    }
