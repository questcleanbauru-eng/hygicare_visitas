...
// Aviso opcional (multi-destinatário) de que um agendamento foi criado.
// Separado de handleCreateAgendamento de propósito: "Repetir a cada 30
// dias" chama createAgendamento várias vezes em loop (um checkpoint
// independente por vez) — notificar dentro daquele handler mandaria um
// push por checkpoint. Quem PODE ser notificado é decidido no front (mesmo
// padrão já usado em Nova Visita/notificarUsuario — admin notifica
// qualquer um, gerente só admins + a própria equipe); aqui só entrega.
export async function handleNotifyAgendamentoCriado(payload) {
    const user = await verifyUser(payload.user);
    ...
    const datas = Array.isArray(payload.datas) ? payload.datas.filter(Boolean) : [];
    const body = datas.length > 1
        ? `${datas.length} agendamento(s) pra ${cliente}, a partir de ${datas[0]}.`
        : `Agendamento pra ${cliente}${datas[0] ? ' em ' + datas[0] : ''}.`;

    let enviados = 0;
    for (const dest of destinatarios) {
        await sendPushToVendedor(dest, {
            title: `📌 ${user.name.split(' ')[0]} criou um agendamento`,
            body, page: 'calendar', tag: `agendamento-criado-${Date.now()}-${enviados}`, tipo: 'aviso'
        });
        enviados++;
    }
