...
export async function handleImportRadarClientesCsv(payload) {
    ...
    for (let r = 1; r < table.length; r++) {
        ...
        if (existing) {
            ...
            updates.push({ rowNumber: existing.rowNumber, rowValues: clientesHeaders.map((h) => (current[h] !== undefined ? current[h] : '')) });
            atualizadas++;
    ...
    if (novasCidades.size) {
        ...
        const cidadeRows = Array.from(novasCidades.values()).map(({ cidade, uf }) => {
            const fields = { Cidade: cidade, Uf: uf, LiberadaEm: hoje, Lat: '', Lng: '' };
