...
// restrita a um vendedor libera só pra aquele email específico.
export function cidadeAcessivel(cidade, user) {
    ...
    if (cidade.restritaVendedorEmail) {
        return cidade.restritaVendedorEmail.trim().toLowerCase() === String(user.email || '').trim().toLowerCase();
    }
...
export async function handleSetCidadeRestricao(payload) {
    ...
    current[keyMap.restritaVendedorEmail] = tipo === 'vendedor' ? String(payload.vendedorEmail || '').trim() : '';
    current[keyMap.restritaVendedorNome] = tipo === 'vendedor' ? String(payload.vendedorNome || '').trim() : '';
