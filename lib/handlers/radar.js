...
// do IBGE) — vive só no servidor, nunca entra no bundle que vai pro
// celular, e resolve a coordenada de uma cidade sem chamar API nenhuma em
// tempo real (o motivo de ter descartado geocodificação via API antes).
let _municipiosCache = null;
function loadMunicipios() {
    if (!_municipiosCache) {
        const filePath = path.join(__dirname, '../data/municipios-br.json');
        _municipiosCache = JSON.parse(readFileSync(filePath, 'utf8'));
    }
