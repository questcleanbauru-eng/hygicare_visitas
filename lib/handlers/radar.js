...
// Reserva de 6 meses: quem agenda prospecção primeiro "trava" a empresa
// pros outros vendedores até essa data ou até concluir (ja_atendido/
// recusado, o que vier primeiro) — evita dois vendedores trabalhando o
// mesmo lead ao mesmo tempo sem saber (a base do Radar é compartilhada
// entre todos com acesso, sem esse limite qualquer um mexe em qualquer
// empresa a qualquer momento).
const RESERVA_MESES = 6;
...
export async function handleUpdateRadarClienteStatus(payload) {
    ...
    // Reserva de 6 meses: quem agendou prospecção primeiro trava a empresa
    // pros outros vendedores até a empresa ser concluída (ja_atendido/
    // recusado) ou a data expirar — admin sempre pode agir, ignora a trava.
    ...
    if (status === 'prospeccao_agendada') {
        const ate = new Date();
        ate.setMonth(ate.getMonth() + RESERVA_MESES);
