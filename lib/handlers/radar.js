...
export async function handleImportRadarClientesCsv(payload) {
    ...
    const idxCnpj = idxOf('cnpj'), idxNome = idxOf('nome'), idxFantasia = idxOf('nomeFantasia'),
        idxCnaeCod = idxOf('cnaeCodigo'), idxCnaeDesc = idxOf('cnaeDescricao'), idxSituacao = idxOf('situacao'),
        idxCidade = idxOf('cidade'), idxUf = idxOf('uf'), idxData = idxOf('dataBusca');
    ...
    for (let r = 1; r < table.length; r++) {
        ...
        if (existing) {
        ...
        } else if (seenNewByCnpj.has(cnpj)) {
            ...
            const idIdx = clientesHeaders.indexOf('Id');
            const id = newRows[i][idIdx];
            const fields = {
                Id: id, Cnpj: cnpj, Nome: nome, NomeFantasia: nomeFantasia, Cidade: cidade, Uf: uf,
                CnaeCodigo: cnaeCodigo, CnaeDescricao: cnaeDescricao, SituacaoCadastral: situacao,
                DataBusca: dataBusca, Status: 'buscado', StatusData: '', StatusMotivo: '',
                StatusRetornoPrevisto: '', VisitaOrigemId: ''
            };
            newRows[i] = clientesHeaders.map((h) => (fields[h] !== undefined ? fields[h] : ''));
            duplicadasNoArquivo++;
        } else {
            const id = String(baseId + newRows.length);
            const fields = {
                Id: id, Cnpj: cnpj, Nome: nome, NomeFantasia: nomeFantasia, Cidade: cidade, Uf: uf,
                CnaeCodigo: cnaeCodigo, CnaeDescricao: cnaeDescricao, SituacaoCadastral: situacao,
                DataBusca: dataBusca, Status: 'buscado', StatusData: '', StatusMotivo: '',
    ...
    const _debugBackfill = { semCoordCount: cidadesSemCoord.length, rows: [] };
    if (cidadesSemCoord.length) {
        ...
        cidadesRowsRaw.forEach((row, i) => {
            ...
            _debugBackfill.rows.push({
                row, latKey, lngKey, latVal: row[latKey], lngVal: row[lngKey],
                latEmpty: String(row[latKey] || '').trim() === '', lngEmpty: String(row[lngKey] || '').trim() === '',
                cidadeKeyName, ufKeyName, cidadeVal: row[cidadeKeyName], ufVal: row[ufKeyName], coordsFound: coords
            });
            if (String(row[latKey] || '').trim() !== '' && String(row[lngKey] || '').trim() !== '') return;
            if (!coords) return;
            const current = { ...row, [latKey]: coords.lat, [lngKey]: coords.lng };
            backfillUpdates.push({ rowNumber: i + 2, rowValues: cidadesHeadersRaw.map((h) => (current[h] !== undefined ? current[h] : '')) });
        });
        _debugBackfill.updatesCount = backfillUpdates.length;
    ...
    return { status: 'success', novas, atualizadas, ignoradas, duplicadasNoArquivo, cidadesAdicionadas, solicitacoesAtendidas, _debugBackfill };
}
