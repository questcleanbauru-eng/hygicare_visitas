...
const CLIENTES_HEADERS = [
    ...
    'Latitude', 'Longitude',
    // Mesmo esquema: preenchidas só pelo script (mesma chamada da
    // geocodificação, sem custo extra de crédito). Sócios e atividades
...
export async function readRadarClienteRows() {
    ...
    return rows.map((row) => ({
        ...
        latitude: s(row, key.latitude),
        longitude: s(row, key.longitude),
