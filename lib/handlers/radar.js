...
// RestritaGerencia/RestritaVendedorEmail: no máximo um dos dois preenchido —
// vazios os dois = cidade liberada pra todo mundo (comportamento de sempre).
// Preenchido um = só quem bate com aquela gerência (equipe inteira) ou
// aquele email (vendedor específico) enxerga a cidade e as empresas dela.
const CIDADES_HEADERS = ['Cidade', 'Uf', 'LiberadaEm', 'Lat', 'Lng', 'RestritaGerencia', 'RestritaVendedorEmail', 'RestritaVendedorNome'];
...
function scoreCapital(value) {
    const normalized = String(value || '').replace(/\./g, '').replace(',', '.').replace(/[^\d.]/g, '');
    const n = Number(normalized);
    if (!Number.isFinite(n)) return 0;
    if (n >= 10000000) return 35;
    if (n >= 1000000) return 30;
    if (n >= 300000) return 22;
    if (n >= 100000) return 15;
    return 8;
}

function scoreRadarCliente(cliente, config) {
    const status = String(cliente.status || 'buscado');
    if (status === 'ja_atendido') return { score: 0, prioridade: 'Encerrada', detalhes: ['Ja e cliente'] };
    if (status === 'recusado') return { score: 0, prioridade: 'Encerrada', detalhes: ['Recusou a abordagem'] };
    const detalhes = [];
    const porte = scoreText(cliente.porte);
    let potencial = porte.includes('grande') ? 35 : porte.includes('medio') ? 25 : porte.includes('pequeno') ? 15 : porte.includes('micro') ? 10 : porte.includes('mei') ? 5 : 0;
    potencial = Math.max(potencial, scoreCapital(cliente.capitalSocial));
    if (potencial) detalhes.push(`${potencial} por potencial/porte`);
    const segmentos = scoreList(config.radar_score_segmentos_prioritarios).map(scoreText);
    const descricaoSegmento = scoreText(`${cliente.segmento || ''} ${cliente.cnaeDescricao || ''} ${cliente.cnaeCodigo || ''}`);
    const segmento = segmentos.some((item) => descricaoSegmento.includes(item)) ? 20 : 0;
    if (segmento) detalhes.push('20 por segmento prioritario');
    const cidades = scoreList(config.radar_score_cidades_prioritarias).map(scoreText);
    const cidade = scoreText(cliente.cidade);
    const cidadeUf = scoreText(`${cliente.cidade || ''}|${cliente.uf || ''}`);
    const regiao = cidades.some((item) => item === cidade || item === cidadeUf) ? 15 : 0;
    if (regiao) detalhes.push('15 por cidade prioritaria');
    const ultimaInteracao = parseDate(cliente.statusData || cliente.dataBusca);
    let interacao = 15;
    if (ultimaInteracao) {
        const dias = Math.max(0, Math.floor((Date.now() - ultimaInteracao.getTime()) / 86400000));
        interacao = dias > 60 ? 15 : dias >= 30 ? 12 : dias >= 7 ? 7 : 3;
    }
    detalhes.push(`${interacao} por ultima interacao`);
    let conversao = status === 'buscado' ? 15 : 10;
    if (cliente.indicadoPor) conversao = Math.min(15, conversao + 5);
    detalhes.push(`${conversao} por chance de conversao`);
    const score = Math.min(100, potencial + segmento + regiao + interacao + conversao);
    return { score, prioridade: score >= 70 ? 'Alta' : score >= 40 ? 'Media' : 'Baixa', detalhes };
}
...
export async function readCidadesRows() {
    ...
    const key = {
        ...
        restritaGerencia: findKey(headers, ['RestritaGerencia']),
        restritaVendedorEmail: findKey(headers, ['RestritaVendedorEmail']),
        restritaVendedorNome: findKey(headers, ['RestritaVendedorNome'])
    };
    ...
    return rows.map((row) => ({
        ...
        restritaGerencia: s(row, key.restritaGerencia),
        restritaVendedorEmail: s(row, key.restritaVendedorEmail),
        restritaVendedorNome: s(row, key.restritaVendedorNome)
    }));
}

// Admin sempre acessa tudo. Sem nenhuma das duas restrições preenchidas, a
// cidade é liberada pra todo mundo (comportamento de sempre, cidades já
// existentes continuam assim até serem atribuídas manualmente). Regra é a
// mesma pra vendedor e gerente — uma cidade restrita à gerência libera pra
// quem tem aquela Gerencia (gerente incluso, já que ele também tem uma);
// restrita a vendedor(es) libera só pra quem está na lista (RestritaVendedorEmail
// guarda os emails separados por vírgula — mais de um vendedor por cidade).
export function cidadeAcessivel(cidade, user) {
    const profile = String(user.profile || '').trim().toLowerCase();
    if (profile === 'admin') return true;
    if (!cidade.restritaGerencia && !cidade.restritaVendedorEmail) return true;
    if (cidade.restritaVendedorEmail) {
        const emails = cidade.restritaVendedorEmail.split(',').map((e) => e.trim().toLowerCase()).filter(Boolean);
        return emails.includes(String(user.email || '').trim().toLowerCase());
    }
    return cidade.restritaGerencia.trim().toLowerCase() === String(user.gerencia || '').trim().toLowerCase();
}
...
export async function handleGetRadarCidadesDisponiveis(payload) {
    ...
    return { status: 'success', cidades: rows.filter((c) => cidadeAcessivel(c, user)) };
}

// Lista completa (sem filtro de acesso), só pra tela de gerenciamento do
// admin atribuir gerência/vendedor por cidade.
export async function handleGetRadarCidadesAdmin(payload) {
    await ensureAdmin(payload.user);
    const rows = await withCache('radar_cidades', 300, () => readCidadesRows());
    return { status: 'success', cidades: rows };
}

// Define (ou limpa, se tipo:'todos') a restrição de uma cidade. Identificada
// por Cidade+Uf — RadarCidadesDisponiveis não tem uma coluna Id própria.
export async function handleSetCidadeRestricao(payload) {
    await ensureAdmin(payload.user);
    const cidade = String(payload.cidade || '').trim();
    const uf = String(payload.uf || '').trim();
    if (!cidade) throw new Error('Cidade obrigatória.');
    const tipo = String(payload.tipo || 'todos').trim().toLowerCase();

    await ensureHeaderColumns(CIDADES_SHEET, CIDADES_HEADERS);
    const headers = await getHeaders(CIDADES_SHEET);
    const rows = await getSheetObjects(CIDADES_SHEET);
    const rowIndex = rows.findIndex((r) => {
        const rCidadeKey = findObjectKey(r, ['cidade']) || 'Cidade';
        const rUfKey = findObjectKey(r, ['uf']) || 'Uf';
        return String(r[rCidadeKey] || '').trim().toLowerCase() === cidade.toLowerCase()
            && String(r[rUfKey] || '').trim().toLowerCase() === uf.toLowerCase();
    });
    if (rowIndex === -1) throw new Error('Cidade não encontrada.');

    const current = rows[rowIndex];
    const keyMap = {
        restritaGerencia: findObjectKey(current, ['restritagerencia']) || 'RestritaGerencia',
        restritaVendedorEmail: findObjectKey(current, ['restritavendedoremail']) || 'RestritaVendedorEmail',
        restritaVendedorNome: findObjectKey(current, ['restritavendedornome']) || 'RestritaVendedorNome'
    };
    // payload.vendedorEmails/vendedorNomes: array (1+ vendedores por cidade).
    // Mantém aceitando vendedorEmail/vendedorNome no singular por
    // compatibilidade, caso algum chamador antigo ainda mande assim.
    const vendedorEmails = Array.isArray(payload.vendedorEmails) ? payload.vendedorEmails
        : payload.vendedorEmail ? [payload.vendedorEmail] : [];
    const vendedorNomes = Array.isArray(payload.vendedorNomes) ? payload.vendedorNomes
        : payload.vendedorNome ? [payload.vendedorNome] : [];

    current[keyMap.restritaGerencia] = tipo === 'gerencia' ? String(payload.gerencia || '').trim() : '';
    current[keyMap.restritaVendedorEmail] = tipo === 'vendedor'
        ? vendedorEmails.map((e) => String(e || '').trim()).filter(Boolean).join(',') : '';
    current[keyMap.restritaVendedorNome] = tipo === 'vendedor'
        ? vendedorNomes.map((n) => String(n || '').trim()).filter(Boolean).join(',') : '';

    await updateRow(CIDADES_SHEET, rowIndex + 2, headers.map((h) => (current[h] !== undefined ? current[h] : '')));
    clearCacheByPrefix(['radar_cidades']);
...
// Set de "cidade|uf" (minúsculo) acessíveis pro usuário — null pra admin
// (sem restrição, mais barato que montar o Set à toa). Usado tanto pra
// validar/filtrar handleGetRadarClientes(scope:'all') quanto pros totais
// do Panorama — servidor sempre revalida, não confia só no filtro do
// dropdown de cidades no front.
async function accessibleCidadeKeySet(user) {
    const profile = String(user.profile || '').trim().toLowerCase();
    if (profile === 'admin') return null;
    const cidades = await withCache('radar_cidades', 300, () => readCidadesRows());
    return new Set(
        cidades.filter((c) => cidadeAcessivel(c, user))
            .map((c) => `${c.cidade.trim().toLowerCase()}|${c.uf.trim().toLowerCase()}`)
    );
}
...
export async function handleGetRadarClientes(payload) {
    ...
    const scored = all.map((cliente) => ({ ...cliente, pontuacao: scoreRadarCliente(cliente, config) }));
    const accessibleKeys = await accessibleCidadeKeySet(user);

    if (payload.scope === 'all') {
        const clientes = accessibleKeys
            ? scored.filter((c) => accessibleKeys.has(`${c.cidade.trim().toLowerCase()}|${c.uf.trim().toLowerCase()}`))
            : scored;
    ...
    if (accessibleKeys && !accessibleKeys.has(`${cidade}|${uf}`)) {
        throw new Error('Você não tem acesso a essa cidade.');
    }
...
export async function handleImportRadarClientesCsv(payload) {
    ...
    // Adiciona colunas novas (Segmento/Endereco/etc, RestritaGerencia/etc)
    // numa planilha já existente sem mexer nas linhas já gravadas —
    // cabeçalho evolui, dado antigo continua lá, só sem valor nessas
    // colunas até o próximo reimport ou até o admin atribuir manualmente.
    await ensureHeaderColumns(CLIENTES_SHEET, CLIENTES_HEADERS);
    await ensureHeaderColumns(CIDADES_SHEET, CIDADES_HEADERS);
    ...
    clearCacheByPrefix(['radar_clientes', 'radar_cidades']);
    return { status: 'success', novas, atualizadas, ignoradas, duplicadasNoArquivo, cidadesAdicionadas };
}

// Cruza CNPJ da Base de Clientes (aba Clientes, já importada por fora) com
// o Radar — empresa do Radar cujo CNPJ já está na Base de Clientes vira
// "já atendido" automaticamente, sem precisar o vendedor achar e marcar
// manualmente uma por uma. Só ANDA NUM SENTIDO (marca match, nunca desfaz
// um "já atendido" existente que não bateu o CNPJ desta vez) — de propósito,
// pra não arriscar derrubar uma empresa que É cliente real só por uma
// divergência de formatação/cadastro na Base.
export async function handleCruzarClientesComRadar(payload) {
    const user = await ensureAdmin(payload.user);

    const clientesRows = await getSheetObjects('Clientes');
    const cnpjSet = new Set();
    clientesRows.forEach((row) => {
        const key = findObjectKey(row, ['cnpj']);
        if (!key) return;
        const cnpj = normalizeCnpj(row[key]);
        if (cnpj) cnpjSet.add(cnpj);
    });
    if (!cnpjSet.size) {
        throw new Error('Nenhum CNPJ encontrado na Base de Clientes — confira se a coluna CNPJ foi importada.');
    }
    ...
    const updates = [];
    rows.forEach((row, i) => {
        const cnpjKey = findObjectKey(row, ['cnpj']) || 'Cnpj';
        const cnpj = normalizeCnpj(row[cnpjKey]);
        if (!cnpj || !cnpjSet.has(cnpj)) return;
        const statusKey = findObjectKey(row, ['status']) || 'Status';
        if (String(row[statusKey] || '').trim().toLowerCase() === 'ja_atendido') return;

        const current = { ...row };
        current[statusKey] = 'ja_atendido';
        current[findObjectKey(current, ['statuspor']) || 'StatusPor'] = user.name;
        current[findObjectKey(current, ['statusporemail']) || 'StatusPorEmail'] = user.email;
        current[findObjectKey(current, ['statusdata']) || 'StatusData'] = formatDate(new Date());
        current[findObjectKey(current, ['statusmotivo']) || 'StatusMotivo'] = '';
        current[findObjectKey(current, ['statusretornoprevisto']) || 'StatusRetornoPrevisto'] = '';
        current[findObjectKey(current, ['indicadopor']) || 'IndicadoPor'] = user.name;
        current[findObjectKey(current, ['indicadoporemail']) || 'IndicadoPorEmail'] = user.email;
        current[findObjectKey(current, ['reservadopor']) || 'ReservadoPor'] = '';
        current[findObjectKey(current, ['reservadoporemail']) || 'ReservadoPorEmail'] = '';
        current[findObjectKey(current, ['reservadoate']) || 'ReservadoAte'] = '';

        updates.push({ rowNumber: i + 2, rowValues: headers.map((h) => (current[h] !== undefined ? current[h] : '')) });
    });

    if (updates.length) await batchUpdateRows(CLIENTES_SHEET, updates);
    clearCacheByPrefix(['radar_clientes']);
    return { status: 'success', clientesComCnpj: cnpjSet.size, atualizados: updates.length };
}
...
// Total da base e as quebras por status/segmento são só das cidades que o
// usuário tem acesso (ver cidadeAcessivel) — admin continua vendo o
// panorama nacional completo. Antes disso existir, "todo mundo vê os
// mesmos números" fazia sentido (não tinha restrição nenhuma); agora que
// cidade pode ser restrita a uma gerência/vendedor, mostrar o total
// nacional pra todo mundo vazaria quantidade/segmento de cidade que a
// pessoa nem deveria enxergar. A lista de "quem está reservado com o quê
// agora" continua filtrada por perfil como já era (vendedor só a própria,
// gerente a equipe, admin tudo), usando a aba Vendedores (fonte de verdade
// de email→gerência) pra saber quem é da equipe do gerente, não
// RadarAcessos (que só sabe quem já abriu o Radar).
export async function handleGetRadarPanorama(payload) {
    const user = await ensureCanAccessRadar(payload.user);
    const all = await withCache('radar_clientes_all', RADAR_CLIENTES_CACHE_TTL, () => readRadarClienteRows());
    const accessibleKeys = await accessibleCidadeKeySet(user);
    const baseParaTotais = accessibleKeys
        ? all.filter((c) => accessibleKeys.has(`${c.cidade.trim().toLowerCase()}|${c.uf.trim().toLowerCase()}`))
        : all;

    const porStatus = { buscado: 0, ja_atendido: 0, recusado: 0, prospeccao_agendada: 0 };
    const porSegmento = {};
    baseParaTotais.forEach((c) => {
    ...
    return {
        status: 'success',
        totalEmpresas: baseParaTotais.length,
        porStatus,
        segmentos,
