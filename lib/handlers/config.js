...
import { ensureAdmin, verifyUser, filterByUser, isDateWithinLastDays, isConfigOn } from '../common.js';
...
export async function ensureCanDelete(user) {
    ...
    const config = await withCache('app_config', 600, () => readEmailConfig());
    if (isConfigOn(config.permitir_apagar_outros)) return u;
...
export async function ensureCanCreateProposalFunil(user) {
    ...
    if (profile === 'admin') return u;
    const config = await withCache('app_config', 600, () => readEmailConfig());
    if (isConfigOn(config.permitir_criar_proposta_funil)) return u;
...
export async function ensureCanAccessRadar(user) {
    ...
    if (profile === 'admin') return u;
    const config = await withCache('app_config', 600, () => readEmailConfig());
    if (!isConfigOn(config.permitir_acesso_radar)) {
