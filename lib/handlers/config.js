...
export async function ensureCanAccessRadar(user) {
    ...
    await ensureRecentVisitActivity(u);
    return u;
}
...
// nos últimos 7 dias. Cache curto (60s): rápido o bastante pra não bater
// na planilha a cada clique, curto o bastante pra liberar o acesso logo
// depois de registrar a visita que faltava, sem esperar minutos.
async function ensureRecentVisitActivity(user) {
    const visits = await withCache('radar_gate_' + user.email, 60, async () =>
        filterByUser(await getSheetObjects('Visitas'), user, 'visits'));
    // Mesma janela de isDateWithinLastDays (usada por handleGetDashboardData
    // pra decidir se mostra o item de nav) — se as duas checagens usassem
    // critérios diferentes, dava pra ver o item de nav e mesmo assim cair
    // num erro ao abrir o Radar (ou vice-versa).
    const temVisitaRecente = visits.some((v) => isDateWithinLastDays(v['Data da Visita'], 7));
    if (!temVisitaRecente) {
        throw new Error('Registre uma visita nos últimos 7 dias pra acessar o Radar de Clientes.');
    }
}
