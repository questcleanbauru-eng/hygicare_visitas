...
// Prioriza o Id (estável, não muda se o e-mail do usuário for editado
// depois e não colide se duas linhas tiverem o mesmo e-mail por engano) —
// cai pro e-mail quando não vem Id (login por PIN sem sessão de admin, ou
// uma sessão antiga que ainda não recarregou a tela de usuários).
async function findVendedorRow(target) {
    const { id, email } = typeof target === 'string' ? { id: '', email: target } : (target || {});
    const headers = await getHeaders('Vendedores');
    const rows = await getSheetObjects('Vendedores');
    const targetId = String(id || '').trim();
    const idx = targetId
        ? rows.findIndex((row) => String(row.Id || '').trim() === targetId)
        : rows.findIndex((row) => normEmail(row.EmailLogin) === normEmail(email));
...
export async function handleAdminSetPin(payload) {
    ...
    if (!id && !target) throw new Error('Informe o usuário.');
    const pin = String(payload.pin || '').trim();
    if (!validPin(pin)) throw new Error('O PIN precisa ter 4 dígitos.');

    await ensureColumns('Vendedores', [...PIN_COLS, 'NomeLogin']);
    const { headers, idx } = await findVendedorRow({ id, email: target });
    if (idx === -1) throw new Error('Usuário não encontrado.');
...
// Remove o PIN. Sem `id`/`email` = o próprio; com um deles apontando pra
// outro usuário = só admin.
export async function handleRemovePin(payload) {
    const user = await verifyUser(payload.user);
    const id = String(payload.id || '').trim();
    const target = normEmail(payload.email || (!id ? user.email : ''));
    const isSelf = !id && target === normEmail(user.email);
    if (!isSelf) {
        await ensureAdmin(payload.user);
    }
