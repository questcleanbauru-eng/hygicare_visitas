    ...
    return withCache('d_' + user.email, 120, async () => {
        ...
        return {
            ...
            data: {
                ...
                // Item de nav só aparece se a mesma regra que o backend vai
                // aplicar de fato (ensureCanAccessRadar) também deixaria passar
                // — senão o vendedor vê "Radar" no menu e cai num erro ao
                // abrir. weeklyVisits já é a mesma janela de 7 dias, filtrada
                // pelo mesmo escopo (filterByUser), então é só reaproveitar.
                canAccessRadar: profile === 'admin' ||
                    (String(appConfig.permitir_acesso_radar || 'false') === 'true' && weeklyVisits > 0)
            }
        };
    });
}
