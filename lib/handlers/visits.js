...
export function normalizeVisitRow(row) {
    return {
        ...
        'TeveDespesas': row['TeveDespesas'] || '',
        'ValorDespesas': row['ValorDespesas'] || '',
        'SyncTimestamp': Number(row.SyncTimestamp) || 0
    };
}

// Check-in por geolocalização foi adicionado depois que a aba Visitas já
// existia — mesmo padrão de ensureFunilAtualizacaoColumn (funil.js): garante
// as colunas uma vez (cache 600s) em vez de checar em toda escrita.
async function ensureVisitasGeoColumns() {
    await withCache('visitas_geo_ensured', 600, async () => {
        const headers = await getHeaders('Visitas');
        const missing = ['Latitude', 'Longitude'].filter((h) => !headers.includes(h));
        if (missing.length) {
            await updateRow('Visitas', 1, [...headers, ...missing]);
        }
        return true;
    });
}

// "Teve Despesas?"/"Valor R$" — liberado só pra quem tem PermDespesas
// (Admin > Usuários), mesmo padrão de coluna adicionada depois da aba já
// existir (ver ensureVisitasGeoColumns acima).
async function ensureVisitasDespesasColumns() {
    await withCache('visitas_despesas_ensured', 600, async () => {
        const headers = await getHeaders('Visitas');
        const missing = ['TeveDespesas', 'ValorDespesas'].filter((h) => !headers.includes(h));
        if (missing.length) {
            await updateRow('Visitas', 1, [...headers, ...missing]);
        }
        return true;
...
function buildVisitRow(headers, payload, id) {
    const map = {
        ...
        'ValorDespesas': payload.teveDespesas === 'Sim' ? (payload.valorDespesas || '') : '',
...
function validateRequiredVisitFields(payload) {
    ...
    if (payload.teveDespesas === 'Sim' && !String(payload.valorDespesas || '').trim()) {
        throw new Error('Informe o valor das despesas.');
    }
}
...
export async function handleGetVisits(payload) {
    ...
    let visits = await withCache(cacheKey, 180, async () => {
        // Leitura crua compartilhada (não por-usuário) — Visitas é a maior
        // aba do app e crescia sem parar de tamanho de cache: cada vendedor
        // pagava sua própria leitura fresca da planilha inteira, e o gate de
        // acesso ao Radar (ensureRecentVisitActivity, config.js) fazia o
        // mesmo de novo, sem compartilhar nada — com muita gente usando ao
        // mesmo tempo isso sozinho já estourava a cota da API do Sheets.
        const rawVisits = await withCache('visitas_sheet_raw', 60, () => getSheetObjects('Visitas'));
        const all = filterByUser(rawVisits.map(normalizeVisitRow), user, 'visits');
...
export async function handleGetVisitById(payload) {
    ...
    const visits = await withCache('v_' + user.email, 180, async () =>
        filterByUser((await withCache('visitas_sheet_raw', 60, () => getSheetObjects('Visitas'))).map(normalizeVisitRow), user, 'visits'));
    const found = visits.find((v) => String(v.ID) === id);
    if (!found) throw new Error('Visita nao encontrada.');
    return { status: 'success', visit: found };
}

// Mesma regra do canLancarDespesas exposto pro front (dashboard.js) — sem
// isso, um payload manipulado na mão poderia gravar despesa mesmo pra quem
// a tela nunca mostra o campo.
function podeLancarDespesas(user) {
    return String(user.profile || '').trim().toLowerCase() === 'admin'
        || String(user.permDespesas || '').trim().toLowerCase() === 'sim';
}

export async function handleCreateVisit(payload) {
    const user = await verifyUser(payload.user);
    const isAdmin = String(user.profile || '').trim().toLowerCase() === 'admin';
    if (!podeLancarDespesas(user)) { payload.teveDespesas = ''; payload.valorDespesas = ''; }
    const escolhido = String(payload.vendedorGerente || '').trim();
    if (isAdmin && escolhido && escolhido.toLowerCase() !== String(user.name || '').trim().toLowerCase()) {
        // Admin registrando a visita em nome de outro vendedor/gerente —
        // mantém o nome escolhido e puxa a gerência dele da aba Vendedores.
        ...
        payload.vendedorGerente = match ? match.NomeVendedor : escolhido;
        payload.gerencia = match ? (match.Gerencia || user.gerencia) : user.gerencia;
    } else {
    ...
    await ensureVisitasGeoColumns();
    await ensureVisitasDespesasColumns();
    const headers = await getHeaders('Visitas');
    ...
    await Promise.all(createdVisits.map((v) => logAudit(user, 'criou', 'visita', v.ID, v.Cliente)));

    // Opcional: quem registra a visita escolhe, na hora, notificar alguém
    // sobre ela (ex.: avisar um coordenador que abriu/fechou uma
    // manutenção) — não é ligado a nenhum tipo de visita específico, é uma
    // escolha por visita.
    const notificarUsuario = String(payload.notificarUsuario || '').trim();
    if (notificarUsuario) {
        const primeira = createdVisits[0];
        // Awaited (não fire-and-forget): numa função serverless o processo
        // pode encerrar assim que a resposta sai, cortando uma promise ainda
        // pendente antes dela terminar de gravar/enviar.
        await sendPushToVendedor(notificarUsuario, {
            title: `📋 ${user.name.split(' ')[0]} registrou uma visita`,
            body: `${primeira.Cliente || 'Cliente'} — ${tiposVisita.join(', ')}`,
            page: 'visit-detail', params: { id: primeira.ID }, tag: 'visita-' + primeira.ID, tipo: 'visita'
        });
    }
...
export async function handleUpdateVisit(payload) {
    ...
    // Edição nunca recaptura geolocalização (o formulário só tenta em Nova
    // Visita) — sem isso, todo "Salvar" numa visita já existente apagaria o
    // check-in original por vir undefined no payload.
    if (finalPayload.latitude === undefined) finalPayload.latitude = current.Latitude || '';
    if (finalPayload.longitude === undefined) finalPayload.longitude = current.Longitude || '';
    // Mesma lógica: quem não vê o campo (não tem PermDespesas) manda o
    // payload sem teveDespesas/valorDespesas — sem isso, editar qualquer
    // outro campo de uma visita que já tinha despesa lançada apagaria o
    // valor. Só bloqueia de verdade quando alguém SEM permissão manda um
    // valor mesmo assim (payload manipulado).
    if (finalPayload.teveDespesas === undefined || !podeLancarDespesas(user)) {
        finalPayload.teveDespesas = current.TeveDespesas || '';
        finalPayload.valorDespesas = current.ValorDespesas || '';
    }
    validateRequiredVisitFields(finalPayload);

    await updateRow('Visitas', rowIndex + 2, buildVisitRow(headers, finalPayload, id));
    clearCacheByPrefix(['v_', 'vp_raw', 'd_', 'visitas_sheet_raw']);
    await logAudit(user, 'editou', 'visita', id, finalPayload.cliente);
...
export async function handleDeleteVisit(payload) {
    ...
    clearCacheByPrefix(['v_', 'vp_raw', 'd_', 'visitas_sheet_raw']);
    await logAudit(user, 'apagou', 'visita', id, cliente);
