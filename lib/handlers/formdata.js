...
import { verifyUser, canAccessClient, isConfigOn } from '../common.js';
import { readEmailConfig } from './config.js';

const FORM_SHEETS = ['Cidades', 'AreasAtuacao', 'PotenciaisCliente', 'Aplicacoes', 'Equipamentos', 'TiposVisita', 'Clientes', 'Vendedores'];
...
export async function handleGetFormData(payload) {
    ...
    const multiTipoVisita = isConfigOn(emailConfig.visita_multi_tipo);
    ...
    // Lista de vendedores/gerentes pro picker "Vendedor / Gerente" da Nova
    // Visita (só o Admin usa o campo, mas a lista em si não é sensível —
    // são nomes + gerência). Ordenada por nome.
    const vendedores = (sheets.Vendedores || [])
        .map((row) => ({
    ...
    return {
        status: 'success',
        data: { cidades, areasAtuacao, potenciaisCliente, aplicacoes, equipamentos, tiposVisita, clientes, vendedores, logoEmpresa, multiTipoVisita }
    };
}
