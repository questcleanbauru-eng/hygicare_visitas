import {
    ...
    updateCell, deleteRow, clearAndWriteColumn, sheetExists, withCache, clearCacheKeys
} from '../sheets.js';
...
export async function handleGetAdminData(payload) {
    ...
    const users = sheets.Vendedores.map((u) => ({
        ...
        PermVerTodasVisitas: u.PermVerTodasVisitas || '', PermDespesas: u.PermDespesas || '',
...
const VENDEDORES_EXTRA_COLUMNS = ['Id', 'MetaVisitasMes', 'PermDelete', 'PermCriarPropostaFunil', 'PermAcessoRadar', 'PermVerTodasVisitas', 'PermDespesas', 'TelasBloqueadas', 'Ativo', 'NomeLogin'];
...
export async function handleSaveUser(payload) {
    ...
    if (id || originalEmail) {
        ...
        const userRow = {
            ...
            PermDespesas: payload.permDespesas !== undefined ? normalizePermOverride(payload.permDespesas) : (rows[rowIndex].PermDespesas || ''),
        ...
        const userRow = {
            ...
            PermDespesas: normalizePermOverride(payload.permDespesas),
    ...
    // Sem isso, nome/gerência/perfil mudado aqui (ex.: virar Gerente) não
    // aparecia nos pickers que usam formData.vendedores (Notificar usuário,
    // Vendedor/Gerente do Admin, destinatário de campanha...) — cada
    // aparelho já tinha isso salvo em localStorage e só reconsulta o
    // servidor quando o cache_version muda (ver bumpCacheVersion/
    // handleForceRefreshData). formdata_all é o cache do lado do servidor;
    // sem clear, ainda serviria a lista velha por até 5min mesmo depois disso.
...
export async function handleSetUserAtivo(payload) {
    ...
    await bumpCacheVersion();
    clearCacheKeys(['admin_all', 'formdata_all', 'user_verify_' + String(rows[rowIndex].EmailLogin || '').trim().toLowerCase()]);
...
export async function handleDeleteUser(payload) {
    ...
    await bumpCacheVersion();
    clearCacheKeys(['admin_all', 'formdata_all', 'user_verify_' + String(alvo.EmailLogin || '').trim().toLowerCase()]);
...
export async function handleSaveLookupList(payload) {
    ...
    const mapping = {
        ...
        // TelefoneDestino/MensagemPadrao/Obrigatorio (roteamento de WhatsApp
        // por tipo) não são mais usados — pode reescrever só a coluna "Tipo"
        // igual as outras listas.
        tiposVisita: { sheet: 'TiposVisita', header: 'Tipo' }
    };
