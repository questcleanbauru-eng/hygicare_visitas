...
export async function handleGetAdminData(payload) {
    ...
    const users = sheets.Vendedores.map((u) => ({
        EmailLogin: u.EmailLogin || '', NomeVendedor: u.NomeVendedor || '', NomeLogin: u.NomeLogin || '', Gerencia: u.Gerencia || '',
...
const VENDEDORES_EXTRA_COLUMNS = ['MetaVisitasMes', 'PermDelete', 'PermCriarPropostaFunil', 'PermAcessoRadar', 'TelasBloqueadas', 'Ativo', 'NomeLogin'];
...
async function findUserRow(email) {
    ...
    const headers = await getHeaders('Vendedores');
    const rows = await getSheetObjects('Vendedores');
    const rowIndex = rows.findIndex((r) => String(r.EmailLogin || '').trim().toLowerCase() === email);
...
export async function handleSetUserAtivo(payload) {
    ...
    if (!email) throw new Error('Informe o usuário.');
    if (email === String(admin.email || '').trim().toLowerCase()) throw new Error('Você não pode desativar a própria conta.');

    const { headers, rows, rowIndex } = await findUserRow(email);
    ...
    clearCacheKeys(['admin_all', 'user_verify_' + email]);
...
export async function handleDeleteUser(payload) {
    ...
    const email = String(payload.emailLogin || payload.originalEmail || '').trim().toLowerCase();
    if (!email) throw new Error('Informe o usuário.');
    if (email === String(admin.email || '').trim().toLowerCase()) throw new Error('Você não pode excluir a própria conta.');

    const { rowIndex, rows } = await findUserRow(email);
    if (rowIndex === -1) throw new Error('Usuário não encontrado.');
