...
// a coluna na planilha se ainda não existir, em vez de exigir que o admin
// mexa na planilha na mão. Usado pelo vínculo Proposta <-> Funil.
async function ensurePropostasVinculoColumn() {
    await withCache('propostas_headers_ensured', 600, async () => {
        const headers = await getHeaders('Propostas');
        if (!headers.includes('FunilVinculado')) {
            await updateRow('Propostas', 1, [...headers, 'FunilVinculado']);
        }
        return true;
    });
}
...
export async function handleUpdateProposal(payload) {
    ...
    if (payload.funilVinculado !== undefined) await ensurePropostasVinculoColumn();
    const headers = await getHeaders('Propostas');
