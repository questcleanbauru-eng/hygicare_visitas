...
import { verifyUser, formatDate, formatDateFromInput, userOwnsRow, resolveCreateId, ensureTextLength } from '../common.js';
import { ensureCanDelete, ensureCanCreateProposalFunil } from './config.js';
import { logAudit } from '../audit.js';
import { uploadToDrive, CONTRATOS_FOLDER_ID } from '../drive.js';

// Sobe o PDF do contrato pra pasta "CONTRATOS APP" no Drive e devolve o
// link de visualização — que vai pro campo Anexo, igual a um link colado.
export async function handleUploadContratoPdf(payload) {
    await verifyUser(payload.user);
    const dataUrl = payload.pdf || payload.dataUrl || '';
    if (!dataUrl) throw new Error('Nenhum PDF recebido.');
    const cliente = String(payload.cliente || '').trim().replace(/[\\/:*?"<>|]/g, '-');
    const stamp = new Date().toISOString().slice(0, 10);
    const nome = `Contrato - ${cliente || 'cliente'} - ${stamp}.pdf`;
    const up = await uploadToDrive({ folderId: CONTRATOS_FOLDER_ID, name: nome, dataUrl, kind: 'pdf' });
    return { status: 'success', anexo: up.viewUrl, fileId: up.id };
}
...
export async function readContratoRows(user) {
    // Compartilhado (não por-usuário) — mesmo motivo do Funil/Agendamentos:
    // quem chama isso envolvia a função inteira num cache por-email, e a
    // leitura da planilha em si nunca era compartilhada entre vendedores.
    const { headers, rows } = await withCache('contratos_sheet_raw', 60, () => getSheetWithHeaders('Contratos'));
...
export async function handleCreateContrato(payload) {
    ...
    ensureTextLength(payload.obs, 'Observações');
    ...
    if (payload._queueRetry) {
        const dup = (await readContratoRows(user)).find((r) => String(r.id) === String(id));
        if (dup) return { status: 'success', contrato: dup };
    }
    ...
    clearCacheByPrefix(['ct_', 'contratos_sheet_raw']);
    await logAudit(user, 'criou', 'contrato', id, payload.cliente);
    return {
        status: 'success',
        contrato: {
...
export async function handleUpdateContrato(payload) {
    ...
    if (!userOwnsRow({ Vendedor: current[keyMap.vend] }, user, 'contratos')) {
        throw new Error('Você não tem permissão para editar este contrato.');
    }
    ensureTextLength(payload.obs, 'Observações');

    const isAdmin = String(user.profile || '').trim().toLowerCase() === 'admin';
    // Só Admin pode reatribuir o dono do contrato — mesmo padrão já usado em
    // Visitas/Propostas/Funil pra impedir manipulação de payload.
    if (payload.ativo !== undefined) current[keyMap.ativo] = payload.ativo;
    if (isAdmin && payload.vendedor !== undefined) current[keyMap.vend] = payload.vendedor;
    ...
    clearCacheByPrefix(['ct_', 'contratos_sheet_raw']);
    await logAudit(user, 'editou', 'contrato', id, current[keyMap.cli]);
    ...
    // novo — mesmo formato que readContratoRows produziria.
    const s = (k) => String((current[k] ?? '') || '');
    const dataKey = findCurrentKey(['data']) || 'Data';
    return {
        status: 'success',
        contrato: {
            id, ativo: s(keyMap.ativo) || 'Sim', data: s(dataKey),
            vendedor: s(keyMap.vend), cliente: s(keyMap.cli), cidade: s(keyMap.cid),
            assinado: s(keyMap.assin), inicio: s(keyMap.inicio), fim: s(keyMap.fim),
            anexo: s(keyMap.anexo), enviarAviso: s(keyMap.aviso) || 'Sim', obs: s(keyMap.obs)
        }
    };
}
...
export async function handleDeleteContrato(payload) {
    ...
    clearCacheByPrefix(['ct_', 'contratos_sheet_raw']);
    await logAudit(user, 'apagou', 'contrato', id, cliente);
