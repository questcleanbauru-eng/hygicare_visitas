...
import { requireUser, verifyUser, getNextId, formatDate, formatDateFromInput } from '../common.js';
import { ensureCanDelete, ensureCanCreateProposalFunil } from './config.js';
import { uploadFileToDrive } from '../drive.js';

const MAX_ANEXO_BASE64_LENGTH = 4_000_000; // ~3MB de PDF, folga dentro do limite de corpo da function da Vercel.
...
async function resolveAnexoLink(payload, currentAnexo) {
    if (!payload.anexoBase64) return currentAnexo || '';
    if (payload.anexoBase64.length > MAX_ANEXO_BASE64_LENGTH) {
        throw new Error('Arquivo muito grande. Envie um PDF de até ~3MB.');
    }
    const uploaded = await uploadFileToDrive({
        base64: payload.anexoBase64,
        filename: payload.anexoFilename || 'contrato.pdf',
        mimeType: payload.anexoMimeType || 'application/pdf'
    });
...
export async function handleCreateContrato(payload) {
    ...
    const anexo = await resolveAnexoLink(payload, '');
...
export async function handleUpdateContrato(payload) {
    ...
    current[keyMap.anexo] = await resolveAnexoLink(payload, current[keyMap.anexo]);
