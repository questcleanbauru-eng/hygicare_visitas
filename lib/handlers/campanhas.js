...
export async function handleCriarCampanha(payload) {
    ...
    if (!['proposta', 'funil', 'visita', 'manutencao'].includes(tipo)) throw new Error('Tipo inválido (proposta, funil, visita ou manutencao).');

    // "visita"/"manutencao" não referenciam registro existente (o vendedor é
    // quem vai criar a Visita/Manutenção ao responder) — o item carrega os
    // dados do rascunho (cliente, cidade...) direto no Itens, em vez de só
    // um Id.
    let itens;
    if (tipo === 'visita') {
...
export async function handleGetCampanha(payload) {
    ...
    // "visita"/"manutencao" não referenciam registro existente — o rascunho
    // já carrega os dados de exibição direto no item (ver handleCriarCampanha).
    if (campanha.tipo === 'visita' || campanha.tipo === 'manutencao') {
