...
export async function handleCriarCampanha(payload) {
    ...
    if (!['proposta', 'funil', 'visita'].includes(tipo)) throw new Error('Tipo inválido (proposta, funil ou visita).');

    // "visita" não referencia registro existente (o vendedor é quem vai
    // criar a Visita ao responder) — o item carrega os dados do rascunho
    // (cliente, cidade...) direto no Itens, em vez de só um Id.
    let itens;
    if (tipo === 'visita') {
...
export async function handleGetCampanha(payload) {
    ...
    // "visita" não referencia registro existente — o rascunho já carrega os
    // dados de exibição direto no item (ver handleCriarCampanha).
    if (campanha.tipo === 'visita') {
        const itensVisita = campanha.itens.map((it) => ({ ...it, tipo: 'visita' }));
        return { status: 'success', campanha: { ...campanha, itens: undefined }, itens: itensVisita };
    }
