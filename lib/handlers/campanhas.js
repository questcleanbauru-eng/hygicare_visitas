...
export async function handleResponderCampanhaItem(payload) {
    ...
    // Avisa por e-mail quem criou a campanha assim que ela é concluída de
    // verdade (todo item respondido) — não a cada item, senão viraria spam
    // numa campanha com vários clientes. Best-effort: um problema no e-mail
    // não pode fazer a resposta do vendedor "falhar" pra ele.
    if (todosOk && current.CriadaPorEmail) {
        try {
            ...
            await sendEmail({ to: current.CriadaPorEmail, subject: `✅ ${user.name} concluiu "${campanha.titulo}"`, html });
