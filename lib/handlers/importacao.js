import {
    ...
    sheetExists, createSheet, clearCacheByPrefix, hideColumn
} from '../sheets.js';
import { ensureAdmin, formatDate } from '../common.js';
import { xlsxToTable, xlsToTable } from './xlsx.js';
...
const PROPOSTAS_SPEC = {
    sheet: 'Propostas',
    ensure: ['SyncTimestamp', 'Resumo'],
    cachePrefixes: ['p_', 'vp_raw', 'd_', 'propostas_sheet_raw'],
    // Mesmo motivo do Funil (ver FUNIL_SPEC.dedupeBy).
    dedupeBy: {
        csv: ['cliente', 'cidade', 'foco', 'data', 'vendedor'],
        sheet: [['Cliente'], ['Cidade'], ['Foco'], ['Data'], ['Vendedor']]
    },
    ...
    cols: {
        ...
        // "Resumo" (itens da proposta, no export novo) tem coluna própria —
        // Observações fica só pra anotação do vendedor.
        resumo: ['resumo'],
    ...
    buildRow(get, ctx) {
        ...
        return {
            ...
            'Produtos': ctx.produtosOverride || get('produtos'),
            'Resumo': get('resumo'),
            'Gerencia': gerencia,
            'Gerência': gerencia,
            'Cidade': ctx.cidadeOverride || get('cidade'),
            // Sem status no arquivo → entra como "AGUARDANDO" (grafia usada na base).
            'Status': get('status') || 'AGUARDANDO',
            // Sem "Atualização" no arquivo → data da importação.
            'Atualização': get('atualizacao') || formatDate(new Date(ctx.now)),
            'Atualizacao': get('atualizacao') || formatDate(new Date(ctx.now)),
            'Hora': '',
            'Atualizar/OBS': obs,
            'Observação': obs,
            'Observacao': obs,
            // Sem data limite no arquivo → 30 dias a partir da importação.
            'Data Limite': get('dataLimite') || formatDate(new Date(ctx.now + 30 * 86400000)),
            // Sem e-mail no arquivo → e-mail do vendedor do cadastro.
            'E-mail': get('email') || (usouCadastro ? (ctx.resolved.email || '') : ''),
            'Email': get('email') || (usouCadastro ? (ctx.resolved.email || '') : ''),
            'SyncTimestamp': ctx.now
        };
    }
};
...
const FUNIL_SPEC = {
    ...
    perClienteVendedor: true,
    // Dedup por conteúdo além do Id: o sistema antigo gera Id novo a cada
    // export, então só o Id não segura reimportação. Mesma combinação
    // abaixo já cadastrada (ou repetida no próprio arquivo) = pula.
    // `sheet` são os cabeçalhos candidatos da aba pra cada campo de `csv`.
    dedupeBy: {
        csv: ['cliente', 'cidade', 'foco', 'data', 'vendedor'],
        sheet: [['Cliente'], ['Cidade'], ['Foco'], ['Data'], ['Vendedor']]
    },
...
const CLIENTES_SPEC = {
    ...
    ensure: ['Nome Fantasia', 'CNPJ'],
    // Existe só pro cruzamento com o Radar (handleCruzarClientesComRadar) —
    // nunca é lida pelo app pra nenhum vendedor, e fica escondida na planilha
    // (ver hideColumns abaixo) pra reduzir exposição de quem abrir a aba
    // direto no Sheets.
    hideColumns: ['CNPJ'],
    ...
    cols: {
        ...
        // Usado pra cruzar com o Radar de Clientes (ver handleCruzarClientesComRadar
        // em lib/handlers/radar.js) — marca automaticamente como "Já é cliente"
        // quem bater o CNPJ.
        cnpj: ['cnpj', 'cnpj/cpf', 'cpf/cnpj', 'cnpj cpf']
...
async function runLegacyImport(payload, spec) {
    ...
    let table;
    if (payload.xlsxBase64 || payload.xlsBase64) {
        const isLegacyXls = !!payload.xlsBase64;
        const b64 = String(payload.xlsBase64 || payload.xlsxBase64);
        ...
        try {
            table = isLegacyXls
                ? await xlsToTable(Buffer.from(b64, 'base64'))
                : xlsxToTable(Buffer.from(b64, 'base64'));
        } catch (e) {
    ...
    vendedores.forEach((v) => {
        ...
        if (n) vendByNorm.set(n, { nome: v.NomeVendedor, gerencia: v.Gerencia || '', email: v.EmailLogin || v.Email || '' });
    });
    ...
    if (!dryRun && spec.hideColumns) {
        // Best-effort — se a API do Sheets falhar aqui (raro), a importação em
        // si não deve travar por causa de um detalhe visual da planilha.
        for (const h of spec.hideColumns) await hideColumn(spec.sheet, h).catch(() => {});
    }
    ...
    // Dedup por conteúdo (spec.dedupeBy): índice do que já está na aba pela
    // combinação de campos, comparando sem acento/caixa/espaço extra.
    const dedupe = spec.dedupeBy || null;
    const existingByContent = new Set();
    const seenContent = new Set();
    if (dedupe && existing.length) {
        const rowKeys = Object.keys(existing[0]);
        const sheetKeys = dedupe.sheet.map((cands) => {
            const wanted = cands.map(normName);
            return rowKeys.find((k) => wanted.includes(normName(k))) || null;
        });
        existing.forEach((r) => {
            const parts = sheetKeys.map((k) => normName(k ? r[k] : ''));
            if (parts[0]) existingByContent.add(parts.join('|'));
        });
    }

    const now = Date.now();
    let novas = 0, atualizadas = 0, puladas = 0, puladasDuplicadas = 0, ignoradas = 0, descartadas = 0;
    ...
    // { "<nº da linha>": "texto digitado em Produtos" } — sobrescreve o que veio do arquivo.
    const linhaProdutosMap = (payload.linhaProdutosMap && typeof payload.linhaProdutosMap === 'object') ? payload.linhaProdutosMap : {};
    ...
    for (let r = 1; r < table.length; r++) {
        ...
        // Mesmo conteúdo já cadastrado (ou repetido no próprio arquivo) com
        // Id diferente: é reimportação, pula em vez de criar de novo. Usa o
        // vendedor já resolvido e as correções de cidade/foco da revisão,
        // que é o que de fato iria pra planilha.
        if (dedupe && !jaExiste) {
            const csvVal = {
                cidade: String(linhaCidadeMap[String(r)] || '').trim() || get('cidade'),
                foco: String(linhaFocoMap[String(r)] || '').trim() || get('foco'),
                vendedor: (effResolved.matched && effResolved.nome) || rawVend
            };
            const parts = dedupe.csv.map((f) => normName(f in csvVal ? csvVal[f] : get(f)));
            const ck = parts[0] ? parts.join('|') : '';
            if (ck && (existingByContent.has(ck) || seenContent.has(ck))) {
                puladasDuplicadas++;
        ...
        const produtosOverride = String(linhaProdutosMap[String(r)] || '').trim();
        const fields = spec.buildRow(get, { resolved: effResolved, focoOverride, cidadeOverride, produtosOverride, idCell, now, seq: newRows.length, payload });
        if (jaExiste) {
    ...
    if (!dryRun) {
        ...
        if (newRows.length || updates.length) {
            clearCacheByPrefix(spec.cachePrefixes);
            // Sem isso, o formData já salvo no aparelho de cada vendedor
            // (ensureFormData, api.js) nunca percebe que a base de Clientes
            // mudou — o app só refaz o fetch quando essa versão muda.
            if (spec.sheet === 'Clientes') await bumpCacheVersion();
        }
    ...
    return {
        status: 'success',
        dryRun,
        resumo: { novas, atualizadas, puladas, puladasDuplicadas, ignoradas, descartadas },
        colunasMapeadas,
...
// Ação avulsa (não depende de reimportar) — cobre CNPJ/CPF que já estava na
// aba Clientes ANTES da coluna passar a ser escondida automaticamente a
// cada import (ver CLIENTES_SPEC.hideColumns acima). Idempotente: reesconder
// o que já está escondido não dá erro.
export async function handleHideClientesCnpj(payload) {
    await ensureAdmin(payload.user);
    await hideColumn('Clientes', 'CNPJ');
