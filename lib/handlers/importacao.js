...
async function runLegacyImport(payload, spec) {
    ...
    for (let r = 1; r < table.length; r++) {
        ...
        // "Desconsiderar essa proposta": o admin escolheu não importar as
        // linhas desse vendedor / desse cliente.
        const descartarPorVend = nRawVend && providedMap.get(nRawVend) === DESCARTAR;
        const descartarPorCli = !nRawVend && spec.perClienteVendedor
            && clienteVendMap.get(clienteKey(get('cliente'))) === DESCARTAR;
        if (descartarPorVend || descartarPorCli) { descartadas++; continue; }
        ...
        // Linha sem vendedor: tenta o vendedor escolhido pro cliente; se ainda
        // não tiver, entra na lista pra o admin escolher.
        let effResolved = resolved;
        if (spec.perClienteVendedor && !nRawVend) {
        ...
        const fields = spec.buildRow(get, { resolved: effResolved, idCell, now, seq: newRows.length, payload });
        if (jaExiste) {
