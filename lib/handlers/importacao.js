...
async function runLegacyImport(payload, spec) {
    ...
    for (let r = 1; r < table.length; r++) {
        ...
        const idCell = get('id');
        if (idCell && (existingIds.has(idCell) || seenIds.has(idCell))) { puladas++; continue; }
        ...
        newRows.push(sheetHeaders.map((h) => (fields[h] !== undefined ? fields[h] : '')));
        if (idCell) seenIds.add(idCell);
    }

    if (!dryRun) {
        if (newRows.length) {
            await appendRows(spec.sheet, newRows);
