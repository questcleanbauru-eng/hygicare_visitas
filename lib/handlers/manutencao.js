...
import { verifyUser, ensureAdmin, formatDate, resolveCreateId, ensureTextLength, isConfigOn, parseDate, canAccessClient, hasBroadDataAccess } from '../common.js';
import { ensureCanDelete, readEmailConfig } from './config.js';
import { logAudit } from '../audit.js';
import { uploadToDrive, deleteDriveFile, FOTOS_MANUTENCAO_FOLDER_ID } from '../drive.js';

const SHEET_NAME = 'Manutencoes';
const HEADERS = [
    'Id', 'Data', 'Tecnico', 'Gerencia', 'Cliente', 'Cidade',
    'ItensTabela', 'Observacao', 'TipoRelatorio', 'TipoManutencao',
    'AssinaturaTecnico', 'AssinaturaCliente', 'ClienteAssinanteNome', 'ClienteAssinanteFuncao',
    'PendenteAprovacao', 'SyncTimestamp', 'Fotos'
];
...
async function ensureManutencoesSheet() {
    ...
    // Migração leve: garante colunas novas (ex.: Fotos) sem tocar nas linhas.
    await withCache('mnt_headers_ensured', 600, async () => {
...
// IDs de arquivo do Drive guardados como lista separada por vírgula.
...
function filterManutencaoByUser(items, user) {
    ...
    if (hasBroadDataAccess(profile)) return items;
    if (profile === 'gerente') {
...
export async function readManutencaoRows(user, options = {}) {
    ...
    const key = {
        ...
        tipoMnt: findKey(headers, ['TipoManutencao', 'TIPOMANUTENCAO']),
        assinTec: findKey(headers, ['AssinaturaTecnico', 'ASSINATURATECNICO']),
        assinCli: findKey(headers, ['AssinaturaCliente', 'ASSINATURACLIENTE']),
        cliAssNome: findKey(headers, ['ClienteAssinanteNome', 'CLIENTEASSINANTENOME']),
        cliAssFuncao: findKey(headers, ['ClienteAssinanteFuncao', 'CLIENTEASSINANTEFUNCAO']),
        pendente: findKey(headers, ['PendenteAprovacao', 'PENDENTEAPROVACAO']),
        sync: findKey(headers, ['SyncTimestamp', 'SYNCTIMESTAMP']),
        fotos: findKey(headers, ['Fotos', 'FOTOS'])
    };
    ...
    const parsed = rows.map((row) => ({
        ...
        tipoManutencao: s(row, key.tipoMnt),
        assinaturaTecnico: s(row, key.assinTec),
        assinaturaCliente: s(row, key.assinCli),
        clienteAssinanteNome: s(row, key.cliAssNome),
        clienteAssinanteFuncao: s(row, key.cliAssFuncao),
        pendenteAprovacao: s(row, key.pendente),
        syncTimestamp: Number(v(row, key.sync)) || 0,
        fotos: parseFotoIds(s(row, key.fotos))
    }));
...
export async function handleCreateManutencao(payload) {
    ...
    const fields = {
        ...
        'TipoRelatorio': ['geral', 'biopet'].includes(payload.tipoRelatorio) ? payload.tipoRelatorio : '',
        ...
        'ClienteAssinanteNome': payload.clienteAssinanteNome || '',
        'ClienteAssinanteFuncao': payload.clienteAssinanteFuncao || '',
        'PendenteAprovacao': '',
        'SyncTimestamp': Date.now(),
        'Fotos': parseFotoIds(Array.isArray(payload.fotos) ? payload.fotos.join(',') : payload.fotos).join(',')
    };
    ...
    return {
        ...
        manutencao: {
            ...
            itensTabela: fields.ItensTabela, observacao: fields.Observacao, tipoRelatorio: fields.TipoRelatorio,
            tipoManutencao: fields.TipoManutencao,
            assinaturaTecnico: fields.AssinaturaTecnico, assinaturaCliente: fields.AssinaturaCliente,
            clienteAssinanteNome: fields.ClienteAssinanteNome, clienteAssinanteFuncao: fields.ClienteAssinanteFuncao,
            pendenteAprovacao: fields.PendenteAprovacao, syncTimestamp: fields.SyncTimestamp,
            fotos: parseFotoIds(fields.Fotos)
        }
    };
}
...
export async function handleUpdateManutencao(payload) {
    ...
    const keyMap = {
        ...
        tipoMnt: findCurrentKey(['tipomanutencao']) || 'TipoManutencao',
        assinTec: findCurrentKey(['assinaturatecnico']) || 'AssinaturaTecnico',
        assinCli: findCurrentKey(['assinaturacliente']) || 'AssinaturaCliente',
        cliAssNome: findCurrentKey(['clienteassinantenome']) || 'ClienteAssinanteNome',
        cliAssFuncao: findCurrentKey(['clienteassinantefuncao']) || 'ClienteAssinanteFuncao',
        pendente: findCurrentKey(['pendenteaprovacao']) || 'PendenteAprovacao',
        sync: findCurrentKey(['synctimestamp']) || 'SyncTimestamp',
        fotos: findCurrentKey(['fotos']) || 'Fotos'
    };
    ...
    const userGer = String(user.gerencia || '').trim().toLowerCase();
    const owns = isAdmin || hasBroadDataAccess(profile) || (profile === 'gerente' && ownerGer === userGer) || ownerName === userName;
    ...
    if (payload.tipoManutencao !== undefined) current[keyMap.tipoMnt] = payload.tipoManutencao;
    ...
    if (payload.clienteAssinanteNome !== undefined) current[keyMap.cliAssNome] = payload.clienteAssinanteNome;
    if (payload.clienteAssinanteFuncao !== undefined) current[keyMap.cliAssFuncao] = payload.clienteAssinanteFuncao;
    if (payload.fotos !== undefined) {
        current[keyMap.fotos] = parseFotoIds(Array.isArray(payload.fotos) ? payload.fotos.join(',') : payload.fotos).join(',');
    }
    ...
    // novo — mesmo formato que readManutencaoRows produziria.
    const s = (k) => String((current[k] ?? '') || '');
    const dataKey = findCurrentKey(['data']) || 'Data';
    const tipoRelKey = findCurrentKey(['tiporelatorio']) || 'TipoRelatorio';
    return {
        status: 'success',
        manutencao: {
            id, data: s(dataKey), tecnico: s(keyMap.tec), gerencia: s(keyMap.ger),
            cliente: s(keyMap.cli), cidade: s(keyMap.cid), itensTabela: s(keyMap.itens),
            observacao: s(keyMap.obs), tipoRelatorio: s(tipoRelKey), tipoManutencao: s(keyMap.tipoMnt),
            assinaturaTecnico: s(keyMap.assinTec), assinaturaCliente: s(keyMap.assinCli),
            clienteAssinanteNome: s(keyMap.cliAssNome), clienteAssinanteFuncao: s(keyMap.cliAssFuncao),
            pendenteAprovacao: s(keyMap.pendente), syncTimestamp: Number(current[keyMap.sync]) || 0,
            fotos: parseFotoIds(s(keyMap.fotos))
        }
    };
}
...
export async function handleDeleteManutencao(payload) {
    ...
    if (!payload.force) {
        ...
        const campanha = await findCampanhaAbertaVinculada(user, 'manutencao', 'manutencaoId', id);
        if (campanha) {
            return {
...
// ── Fotos (Google Drive) ────────────────────────────────────────────────
// O cliente comprime a imagem no navegador (~1280px / JPEG) e manda a data
// URL; aqui a gente sobe pra pasta "FOTO MANUTENÇÃO" e devolve o id/URLs.
// Se um id de relatório for passado, já anexa na linha; senão o cliente
// segura os ids e manda em `fotos` no createManutencao.
export async function handleUploadManutencaoFoto(payload) {
    const user = await verifyUser(payload.user);
    const dataUrl = payload.imagem || payload.dataUrl || '';
    if (!dataUrl) throw new Error('Nenhuma imagem recebida.');

    const cliente = String(payload.cliente || '').trim();
    const stamp = new Date().toISOString().slice(0, 19).replace(/[:T]/g, '-');
    const nome = `${cliente ? cliente.replace(/[\\/:*?"<>|]/g, '-') + ' - ' : ''}${stamp}.jpg`;

    const up = await uploadToDrive({
        folderId: FOTOS_MANUTENCAO_FOLDER_ID,
        name: nome,
        dataUrl,
        kind: 'image'
    });
    ...
    if (id) {
        ...
        if (rowIndex !== -1) {
            const current = rows[rowIndex];
            const fotosKey = Object.keys(current).find((k) => k.toLowerCase() === 'fotos') || 'Fotos';
            const syncKey = Object.keys(current).find((k) => k.toLowerCase() === 'synctimestamp') || 'SyncTimestamp';
            const ids = parseFotoIds(current[fotosKey]);
            ids.push(up.id);
            current[fotosKey] = ids.join(',');
    ...
    return { status: 'success', foto: up };
}

export async function handleDeleteManutencaoFoto(payload) {
    const user = await verifyUser(payload.user);
    const fotoId = String(payload.fotoId || '').trim();
    if (!fotoId) throw new Error('Informe a foto.');
    ...
    if (id) {
        ...
        if (rowIndex !== -1) {
            ...
            const fotosKey = Object.keys(current).find((k) => k.toLowerCase() === 'fotos') || 'Fotos';
            const syncKey = Object.keys(current).find((k) => k.toLowerCase() === 'synctimestamp') || 'SyncTimestamp';
            current[fotosKey] = parseFotoIds(current[fotosKey]).filter((x) => x !== fotoId).join(',');
            current[syncKey] = Date.now();
            await updateRow(SHEET_NAME, rowIndex + 2, headers.map((h) => (current[h] !== undefined ? current[h] : '')));
            clearCacheByPrefix(['mnt_', 'manutencao_sheet_raw']);
        }
...
const MODELOS_HEADERS = ['Id', 'Cliente', 'Nome', 'ItensTabela', 'TipoRelatorio', 'AtualizadoPor', 'Gerencia', 'AtualizadoEm'];
...
async function ensureModelosSheet() {
    ...
    // Migração: garante colunas novas (Nome, Gerencia) sem mexer nas linhas
    // já salvas — ficam vazias, caem nos fallbacks abaixo (Nome → nome do
    // cliente; Gerencia vazia nunca bate com a de ninguém, então um modelo
    // antigo só continua visível pra quem já enxergava tudo).
    await withCache('mnt_modelos_headers_ensured', 600, async () => {
        ...
        if (missing.length) await updateRow(MODELOS_SHEET_NAME, 1, [...headers, ...missing]);
        return true;
    });
}
...
// Modelo não tem "dono" de verdade — Gerencia/AtualizadoPor são só um
// retrato de quem salvou por último (upsert reescreve a linha toda). Mesma
// regra de visibilidade dos relatórios (filterManutencaoByUser acima):
// admin/tecnico/administrativo vê tudo, gerente vê a própria gerência,
// vendedor só o que ele mesmo salvou por último.
function filterModelosByUser(rows, user) {
    const profile = String(user.profile || '').trim().toLowerCase();
    if (hasBroadDataAccess(profile)) return rows;
    if (profile === 'gerente') {
        const userGer = String(user.gerencia || '').trim().toLowerCase();
        return rows.filter((r) => String(r.Gerencia || '').trim().toLowerCase() === userGer);
    }
    const userName = String(user.name || '').trim().toLowerCase();
    return rows.filter((r) => String(r.AtualizadoPor || '').trim().toLowerCase() === userName);
}

export async function handleGetManutencaoModelos(payload) {
    const user = await verifyUser(payload.user);
    await ensureModelosSheet();
    const allRows = await withCache('mnt_modelos_all', 300, () => getSheetObjects(MODELOS_SHEET_NAME));
...
export async function handleSaveManutencaoModelo(payload) {
    ...
    const fields = {
        ...
        Nome: nome,
        ItensTabela: payload.itensTabela || '[]',
        TipoRelatorio: ['biopet'].includes(payload.tipoRelatorio) ? payload.tipoRelatorio : '',
    ...
    return {
        status: 'success',
        modelo: { id: String(fields.Id), cliente: fields.Cliente, nome: fields.Nome, itensTabela: fields.ItensTabela, tipoRelatorio: fields.TipoRelatorio }
    };
}
