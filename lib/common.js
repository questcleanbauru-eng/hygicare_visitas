...
// Toggle de config (ConfigEmail, Chave/Valor "true"/"false") pode voltar da
// API do Sheets como "TRUE" (maiúsculo) em vez do "true" que o app grava —
// acontece quando a célula vira um tipo booleano de verdade na planilha
// (ex.: alguém editou a célula direto no Sheets, ou um checkbox), e a
// leitura por padrão (FORMATTED_VALUE) devolve o texto como a planilha
// exibe, não o literal original. Comparação `=== 'true'` sem isso falha
// silenciosamente — o toggle aparece ligado na planilha mas o código nunca
// reconhece como ligado.
export function isConfigOn(value) {
...
export async function verifyUser(user) {
    ...
    if (String(found.Ativo || '').trim().toLowerCase() === 'nao') {
        throw new Error('Conta desativada. Fale com o administrador.');
    }
    return {
        email: found.EmailLogin, name: found.NomeVendedor, profile: found.Perfil, gerencia: found.Gerencia,
        // Overrides individuais (por vendedor) das 3 permissoes que ate aqui so
        // existiam como chave global — vazio = "sem override, usa a config
        // global"; 'Sim'/'Nao' sobrescreve a config global so pra esse usuario.
        permDelete: found.PermDelete || '', permCriarPropostaFunil: found.PermCriarPropostaFunil || '',
        permAcessoRadar: found.PermAcessoRadar || '',
        permVerTodasVisitas: found.PermVerTodasVisitas || '',
        permDespesas: found.PermDespesas || '',
        // Telas escondidas pra esse usuário (Admin → editar usuário).
        telasBloqueadas: String(found.TelasBloqueadas || '').split(',').map((s) => s.trim()).filter(Boolean)
    };
}

// Resolve uma permissao levando em conta (em ordem): admin sempre pode,
// depois o override individual do usuario (se houver), e por ultimo a
// config global (comportamento de sempre, quando nao ha override).
export function resolvePermission(profile, overrideValue, globalOn) {
    if (String(profile || '').trim().toLowerCase() === 'admin') return true;
    const ov = String(overrideValue || '').trim().toLowerCase();
    if (ov === 'sim') return true;
    if (ov === 'nao' || ov === 'não') return false;
    return !!globalOn;
}

// Perfis "operacionais" que não fazem parte da hierarquia de vendas
// (Admin/Gerente/Vendedor) mas precisam achar/acessar qualquer cliente e o
// que está registrado pra ele — Visitas, Funil, Propostas, Manutenções —
// mesmo sem gerência nem vínculo de vendedor. NÃO dá os poderes de Admin
// (painel Admin, apagar, configurações) — só a mesma visão ampla que
// Admin/Gerente já têm nessas listagens.
export function hasBroadDataAccess(profile) {
    const p = String(profile || '').trim().toLowerCase();
    return p === 'admin' || p === 'tecnico' || p === 'administrativo';
}
...
export function canAccessClient(row, user) {
    ...
    if (profile === 'gerente' || hasBroadDataAccess(profile)) return true;
...
export function userOwnsRow(row, user, type) {
    const profile = String(user.profile || '').trim().toLowerCase();
    if (hasBroadDataAccess(profile)) return true;
...
export function filterByUser(items, user, type) {
    ...
    if (type === 'visits') {
        // Override individual (Admin → editar usuário → "Ver visitas de
        // todos") pra quem precisa enxergar tudo sem ganhar o resto que vem
        // junto com o perfil Admin (painel Admin, Campanhas...).
        if (String(user.permVerTodasVisitas || '').trim().toLowerCase() === 'sim') return items;
        if (profile === 'gerente') {
